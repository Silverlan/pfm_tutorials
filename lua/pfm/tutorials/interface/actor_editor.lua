...
time.create_simple_timer(0.1, function()
	gui.Tutorial.start_tutorial("actor_editor")
end)
...
gui.Tutorial.register_tutorial("actor_editor", function(elTut, pm)
	...
	elTut:RegisterSlide("ae_scenegraph", {
		init = function(slideData, slide)
			...
			slide:AddMessageBox(
				"The most important part of the actor editor is the scene graph, where all of the actors are listed in a tree structure."
			)
			print(item)
		end,
	...
	elTut:RegisterSlide("ae_actor", {
		init = function(slideData, slide)
			...
			slide:SetFocusElement(actorEditor:GetTree())
			slide:AddMessageBox(
				'These are actors. An actor can be anything from a light source to a prop or the sky. Depending on the context, actors may also be referred to as "entities" or "game objects".'
			)
		end,
	...
	elTut:RegisterSlide("ae_components", {
		init = function(slideData, slide)
			...
			slide:AddMessageBox(
				'Every actor is made up of components, which define the behavior of the actor. For instance, a camera is just a normal actor with a "camera" component attached to it. If you were to remove the component, it would no longer be a camera.\nYou can add new components by right-clicking an actor.\n\nCustom components created using the Lua API will also appear here.'
			)
		end,
	...
	elTut:RegisterSlide("ae_component", {
		init = function(slideData, slide)
			...
			slide:AddHighlight(item:GetChildContainer())
			slide:AddMessageBox(
				'Most components have properties, which can be changed to affect its behavior. You can edit a property by clicking on it.\nClick on the "Field of View" property to continue.'
			)
		end,
	...
	elTut:RegisterSlide("ae_property", {
		init = function(slideData, slide)
			...
			slide:AddMessageBox(
				"This is the current value for the property you have selected. You can click it to change its value. Depending on the type of property, you may get a different input field (e.g. a text input or a slider).\nMost properties can also be animated, with some exceptions (such as text-based properties)."
			)
		end,
