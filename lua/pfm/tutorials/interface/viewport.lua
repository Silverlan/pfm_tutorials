...
time.create_simple_timer(0.1,function() gui.Tutorial.start_tutorial("viewport") end)
...
gui.Tutorial.register_tutorial("viewport",function(elTut,pm)
	...
	elTut:RegisterSlide("viewport_interaction1",{
		init = function(slideData,slide)
			...
			slide:AddMessageBox("To control the viewport camera, press and hold the right mouse button, which will enable FPS controls. As long as you hold down the button, you can rotate the camera by moving the mouse, and move the camera using WASD. You can also change the field of view using the scroll wheel.\n\nTry moving the camera until you find the origami cat in the dining room.")

			slideData.targetEntity = ents.find_by_uuid(uuidCat)
			if(util.is_valid(slideData.targetEntity) == false) then return end
		...
		clearCondition = function(slideData)
			if(util.is_valid(slideData.gameObjectOutline) == false) then return true end
			if(slideData.gameObjectOutline:IsTargetInFullView() == false) then return false end
			local cam = game.get_render_scene_camera()
			if(util.is_valid(cam) == false or util.is_valid(slideData.targetEntity) == false) then return true end
			local trC = cam:GetEntity():GetComponent(ents.COMPONENT_TRANSFORM)
			if(trC == nil) then return true end
	...
	elTut:RegisterSlide("viewport_interaction2",{
		init = function(slideData,slide)
			slide:AddHighlight(slide:FindElementByPath("pfm_primary_viewport/viewport"))
			slide:AddMessageBox("Now click the cat with your left mouse button to select it. Selected objects have a yellow outline around them.\nYou cannot select objects that are part of the map.")
			slideData.targetEntity = ents.find_by_uuid(uuidCat)
			pm:DeselectAllActors()
		end,
		clearCondition = function(slideData)
			if(util.is_valid(slideData.targetEntity) == false) then return true end
	...
	elTut:RegisterSlide("viewport_manip_controls",{
		init = function(slideData,slide)
			...
			slide:AddMessageBox("These are the manipulator controls, which can be used to manipulate selected actors, bones, etc. in the scene. From left to right:\n- Select (" .. 
		...
		clearCondition = function(slideData)
			local vp = tool.get_filmmaker():GetViewport()
			if(util.is_valid(vp) == false) then return true end
	...
	elTut:RegisterSlide("viewport_move_actor",{
		init = function(slideData,slide)
			slide:AddHighlight(slide:FindElementByPath("pfm_primary_viewport"))
			slide:AddMessageBox("Now try moving the actor into the marked spot on the table in the living room using the transform gizmo.")
		...
		clearCondition = function(slideData)
			if(util.is_valid(slideData.targetEntity) == false or util.is_valid(slideData.locationMarker) == false) then return true end
	...
	elTut:RegisterSlide("viewport_manip_controls_fin",{
		init = function(slideData,slide)
			slide:SetFocusElement(slide:FindElementByPath("pfm_primary_viewport"))
			slide:AddHighlight(slide:FindElementByPath("pfm_primary_viewport/manip_controls"))
			slide:AddMessageBox("You can toggle the transform space between world, local and screen space by pressing the transform button multiple times.")
		end,
	...
	elTut:RegisterSlide("viewport_live_render",{
		init = function(slideData,slide)
			...
			slide:AddMessageBox("The \"Live Render\" setting toggles the viewport to a real-time raytraced preview of the scene, rendered using the Cycles renderer. This can be useful to get a quick preview of what your scene will look like with baked lightmaps, or when using the Cycles renderer to render the final image.\nYou can still move around the scene with the \"Live Render\" mode active, but some actions (like adding or removing actors) will not be visible until the mode is deactivated.")
		end,
	...
	elTut:RegisterSlide("cc_controls",{
		init = function(slideData,slide)
			...
			slide:AddMessageBox("These are the camera controls. Here you toggle between the work and scene camera, or change the scene camera to a different one.\n\nSwitch to the scene camera to continue.")
		end,
		clearCondition = function(slideData)
			local vp = tool.get_filmmaker():GetViewport()
			if(util.is_valid(vp) == false) then return true end
	...
	elTut:RegisterSlide("viewport_playback_controls",{
		init = function(slideData,slide)
			...
			slide:AddMessageBox("These are the playback controls. Here you can view your animation frame-by-frame, or in real-time by pressing the play-button. Some keybindings are also available:\n" ..
				pfm.get_key_binding("pfm_action toggle_play") .. ": Toggle Play\n" ..
				pfm.get_key_binding("pfm_action previous_frame") .. ": Previous Frame\n" ..
				pfm.get_key_binding("pfm_action next_frame") .. ": Next Frame\n" ..
				pfm.get_key_binding("pfm_action previous_bookmark") .. ": Previous Bookmark\n" ..
				pfm.get_key_binding("pfm_action next_bookmark") .. ": Next Bookmark\n" ..
				pfm.get_key_binding("pfm_action create_bookmark") .. ": Create Bookmark\n\n" ..
